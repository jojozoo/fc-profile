...
class CssValidator {
     ...
     */
    public CssValidator(HtmlCorrectRule policy) {
        this.policy = policy;
    }
     ...
     * @param results
     *            the <code>CleanResults</code> object to add any error messages
     *            to
    ...
    public boolean isValidSelector(String selectorName, Selector selector)
        ...
        switch (selector.getSelectorType()) {
            ...
            case Selector.SAC_DESCENDANT_SELECTOR:
                // these are compound selectors - decompose into simple
                // selectors
            ...
            default:
                throw new ProfileException(HTMLEntityEncoder.htmlEntityEncode(selector.toString()));
        }
    }
    ...
    private boolean validateSimpleSelector(SimpleSelector selector) {
        ...
        return policy.getRegularExpression("cssElementSelector").getPattern()
                .matcher(selector.toString().toLowerCase()).matches()
                & !policy.getRegularExpression("cssElementExclusion")
                        .getPattern()
                        .matcher(selector.toString().toLowerCase()).matches();
    }
    ...
    public boolean isValidCondition(String selectorName, Condition condition)
        ...
        switch (condition.getConditionType()) {
            ...
            case Condition.SAC_OR_CONDITION:
                // these are compound condition - decompose into simple
                // conditions
            ...
            case Condition.SAC_PSEUDO_CLASS_CONDITION:
                ...
                // against valid pattern and is not blacklisted by exclusion
                // pattern
                return validateCondition((AttributeCondition) condition, policy
            ...
            default:
                throw new ProfileException(HTMLEntityEncoder.htmlEntityEncode(selectorName));
        }
    }
    ...
    private boolean validateCondition(AttributeCondition condition,
            RegexPattern pattern, RegexPattern exclusionPattern) {
    ...
    /**
     * Converts the given lexical unit to a <code>String</code> representation.
     * This method does not perform any validation - it is meant to be used in
     * conjunction with the validator/logging methods.
     * 
     * @param lu
