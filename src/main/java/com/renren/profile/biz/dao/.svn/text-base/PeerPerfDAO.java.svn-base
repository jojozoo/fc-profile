...
public interface PeerPerfDAO {
  ...
  String FIELD_PK = "id";
  String FIELDS_WITHOUT_PK = "peer_id,is_leader,invitation_id,user_perf_id,perf_time_id,content,advantage_comments,disadvantage_comments,status,edit_time,is_promotion,promotion_comment,perf_score";
   ...
   * @return
   */
  @SQL("select "+FIELDS_ALL+" from "+TABLE+" where `peer_id` = :1 and  `perf_time_id` = :2 and `is_leader` = :3 and  `user_perf_id` in (:4)")
  public Collection<PeerPerf> batchQueryPeerPerfInfo(int userid,int perfTimeId,int isLeader,Collection<Integer> perfIds);
  ...
  String SQL_UPDATE_MODEL_FILEDS_WITHOUT_PK = "peer_id=:model.peerId,is_leader=:model.isLeader,invitation_id=:model.invitationId,user_perf_id=:model.userPerfId,perf_time_id=:model.perfTimeId,content=:model.content,advantage_comments=:model.advantageComments,disadvantage_comments=:model.disadvantageComments,status=:model.status,edit_time=:model.editTime,is_promotion=:model.isPromotion,promotion_comment=:model.promotionComment,perf_score=:model.perfScore";
  
  @SQL("update "+TABLE+" set "+SQL_UPDATE_MODEL_FILEDS_WITHOUT_PK+" where id=:1.id")
  public int update(@SQLParam("model") PeerPerf model);
  
  @SQL("insert ignore into " + TABLE +" ("+FIELDS_WITHOUT_PK+") VALUES (:model.peerId,:model.isLeader,:model.invitationId,:model.userPerfId,:model.perfTimeId,:model.content,:model.advantageComments,:model.disadvantageComments,:model.status,:model.editTime,:model.isPromotion,:model.promotionComment,:model.perfScore)")
	@ReturnGeneratedKeys
