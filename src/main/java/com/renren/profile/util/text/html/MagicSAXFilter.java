...
import com.xiaonei.admin.util.sys.AdminException;
import com.xiaonei.admin.util.text.html.ErrorMessageUtil;
import com.xiaonei.admin.util.text.html.ParentTag;
 ...
 * 
 */
class MagicSAXFilter extends DefaultFilter implements XMLDocumentFilter {
	...
	private HtmlCorrectRule htmlCorrectRule;
	private ResourceBundle messages;

	private boolean isNofollowAnchors;
	private boolean isValidateParamAsEmbed;
	private boolean inCdata = false;
    private String actionWhenAgainParentTags;
	
	public MagicSAXFilter(HtmlCorrectRule instance, ResourceBundle messages) {
		this.htmlCorrectRule = instance;
		this.messages = messages;
		
		isNofollowAnchors = "true".equals(htmlCorrectRule.getDirective(HtmlCorrectRule.ANCHORS_NOFOLLOW));
		isValidateParamAsEmbed = "true".equals(htmlCorrectRule.getDirective(HtmlCorrectRule.VALIDATE_PARAM_AS_EMBED));
		actionWhenAgainParentTags = htmlCorrectRule.getDirective(HtmlCorrectRule.ACTION_WHEN_AGAIN_PARENT_TAGS);
		if(actionWhenAgainParentTags == null){
		    actionWhenAgainParentTags = HtmlCorrectRule.DEFAULT_ACTION_WHEN_AGAIN_PARENT_TAGS;
		}
	}
	...
	public void comment(XMLString text, Augmentations augs) throws XNIException {
	    String preserveComments = htmlCorrectRule.getDirective(HtmlCorrectRule.PRESERVE_COMMENTS);
	...
	public void endElement(QName element, Augmentations augs) throws XNIException {
		if (!operations.empty() && "remove".equals(operations.peek())) {
			...
			try {
				CleanResults results = cssScanner.scanStyleSheet(cssContent.toString(), htmlCorrectRule.getMaxInputSize());
	...
	private CssScanner makeCssScanner() {
		if (cssScanner == null) {
			cssScanner = new CssScanner(htmlCorrectRule, messages);
		}
		return cssScanner;
	}
	...
	public void startElement(QName element, XMLAttributes attributes, Augmentations augs) throws XNIException {
		...
		Tag tag = htmlCorrectRule.getTagByName(element.localpart);
		...
		if (tag == null && isValidateParamAsEmbed && "param".equals(element.localpart.toLowerCase())) {
			Tag embedPolicy = htmlCorrectRule.getTagByName("embed");
			if (embedPolicy != null && HtmlCorrectRule.ACTION_VALIDATE.equals(embedPolicy.getAction())) {
		...
		if (!operations.isEmpty() && ("remove".equals(operations.peek()) || "css".equals(operations.peek()))) {
		...
		} else if ((tag == null && "encode".equals(htmlCorrectRule.getDirective("onUnknownTag"))) || (tag != null && "encode".equals(tag.getAction()))) {
			...
			if (isStyle) {
				...
				// 验证父亲Tag
				List<ParentTag> parentTags = tag.getParentTagNames();
				if(parentTags != null && parentTags.size() > 0){// 限制了父亲节点
				    boolean hasParentTag = false;
				    // 遍历所有父亲节点
				    int tagNameCount = tagNames.size();
				    for(ParentTag parentTag : parentTags){
				        String parentTagName = parentTag.getValue();
                        int maxDistance = Math.min(tagNameCount,parentTag.getDistance());
				        for(int tagNo = 0;tagNo < maxDistance; ++ tagNo){
	                        String tagName = tagNames.elementAt(tagNameCount - 1 - tagNo);
	                        if(parentTagName.equals(tagName)){// ok
	                            hasParentTag = true;
	                            break;
	                        }
	                    }
				    ...
				    if(!hasParentTag){
				        if("remove".equals(actionWhenAgainParentTags)){
				            removeTag = true;
				        }else if("filter".equalsIgnoreCase(actionWhenAgainParentTags)){
				            filterTag = true;
				        }
				    }
				}
				...
				for (int i = 0; i < attributes.getLength(); i++) {
					...
					if (attribute == null) {
						...
						attribute = htmlCorrectRule.getGlobalAttributeByName(name.toLowerCase());
					}
					// boolean isAttributeValid = false;
					if ("style".equalsIgnoreCase(name)) {
						CssScanner styleScanner = makeCssScanner();
						try {
							CleanResults cr = styleScanner.scanInlineStyle(value, element.localpart, htmlCorrectRule.getMaxInputSize());
		...
		tagNames.push(element.localpart);
	}
