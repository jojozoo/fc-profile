		...
		<div id="content">
			...
			<p>角色:
				<c:forEach var="role" items="${all_roles}" varStatus="status">
					<label><input type="checkbox" name="roles" value="${role.id }" ${role_set != null && role_set.contains(role.id) ? 'checked="checked"' : '' }>${role.name }</label>
				</c:forEach>
			</p>
			<p><input type="submit" name="button" value="保存"/><span node-type="form-message"></span></p>
			</form>
		</div>
		<div class="fc"></div>
	</div>
