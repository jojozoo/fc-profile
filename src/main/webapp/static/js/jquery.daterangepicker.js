...
jQuery.fn.daterangepicker = function(settings){
	...
	var options = jQuery.extend({
		presetRanges: [
			...
			{text: '过去的7天', dateStart: 'today-7days', dateEnd: 'today' },
			{text: '本月', dateStart: function(){ return Date.parse('today').moveToFirstDayOfMonth();  }, dateEnd: 'today' },
			{text: '今年', dateStart: function(){ var x= Date.parse('today'); x.setMonth(0); x.setDate(1); return x; }, dateEnd: 'today' },
			//extras:
		...
		presets: {
			specificDate: '指定日期', 
			allDatesBefore: '之前的所有日期', 
			allDatesAfter: '之后的所有日期', 
			dateRange: '日期范围'
		},
